---
title: "Potability of Water: The Effect on Health and Safe Water Consumption"
author: "Team 5: Katie Hu, Andrew Abeles, Emma Oo, Jake Burnett"
date: "8/16/2021"
...
### k-Nearest Neighbors Equation

$$\hat y  = arg \ max_v \sum_{(x_i , y_i) \in D_z}^{i} I(v = y_i)$$
...
model_metrics = data.frame(
  ...
  k_NN = c(".65", ".62", ".34", ".86", ".66", ".56", ".34", ".84"),
  CART = c(".62", ".58", ".19", ".91",  ".66", ".60", ".21", ".92"),
  C5.0 = c(".63", ".64", ".17", ".94", ".64", ".53", ".28", ".85"),
  LR = c(".53", ".41", ".41", ".61", ".57", ".37", ".21", ".79")
)
```

```{r}
kable(model_metrics, format = 'html', caption = '<b>Model Metrics<b>', digits = c(0,0,2,3), align= 'lcccc')%>%
  kable_styling(full_width= TRUE , position = 'left')%>%
  add_header_above(c('With Outliers' = 5), align = 'l', bold = TRUE)%>%
  pack_rows('Without Outliers',5,8, label_row_css = 'c', indent = F)%>%
  row_spec(row = 0, align = "c", bold = TRUE)%>%
