...
```{r}
#Plot and Determine Area under Curve
roc_knn <- class::knn(
  train = X_train,
  test = X_test, 
  cl = y_train, 
...
a <- attributes(roc_knn)$prob
roc(y_test, a)

plot(roc(y_test, a), print.auc = T, print.auc.y = 0.2, main = "k-NN AUC", legacy.axes = TRUE)
```
...
plot(roc(y_test, a), print.auc = T, print.auc.y = 0.2, main = "k-NN AUC")
```
...
#ggPredict(logreg02_test_no_outliers, se=TRUE, interactive=TRUE, digits=3)
```
...
round(accuracy_no_outliers,2)
round(precision_no_outliers,2)
round(sensitivity_no_outliers,2)
round(specificity_no_outliers,2)
```

```{r}
#Logistic Regression AUC
auc(water_test$Potability, predicted)
```
